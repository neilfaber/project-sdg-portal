...
urlpatterns = [
    # Team endpoints
    path('', StudentTeamViewSet.as_view({'get': 'list', 'post': 'create'}), name='team-list'),
    path('assigned/', StudentTeamViewSet.as_view({'get': 'assigned'}), name='faculty-assigned-teams'),
    path('<int:pk>/', StudentTeamViewSet.as_view({
        'get': 'retrieve',
        'put': 'update',
        'patch': 'partial_update',
        'delete': 'destroy'
    }), name='team-detail'),
    path('<int:pk>/add_member/', StudentTeamViewSet.as_view({'post': 'add_member'}), name='team-add-member'),
    path('<int:pk>/remove_member/', StudentTeamViewSet.as_view({'delete': 'remove_member'}), name='team-remove-member'),
    path('<int:pk>/members/', StudentTeamViewSet.as_view({'get': 'members'}), name='team-members'),
